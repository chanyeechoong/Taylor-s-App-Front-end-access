...
<script>
		...
		function update_elements_and_action(insertID,updateID,hiddenID){
			...
			if (insertElement.checked && x == 0) {
				hiddenElement.setAttribute('value','insert_campus_news');
			...
			if (updateElement.checked && x == 1) {
				...
				hiddenElement.setAttribute('value','update_campus_news');
...
<body>
	...
	<table class = "tableDesign">
		<caption>Campus News List</caption>
		<thead>
			<tr>
				<th>ID</th>
				<th>Title</th>
		...
		<?php foreach ($result as $value):
			$id = $value['id'];
			$title = $value['title'];
			$anewsContent = $value['cnewsContent'];
...
<div id="form-wrapper" class="form-wrapper">
		...
		<form id="form-action" action="../." method="post">
			...
			<input type="hidden" id="hidden_action" name="action" value="insert_campus_news">

			<div id="action" class="radio-group">
				<input type="radio" name="insert_or_update" id="insert" value="insert_campus_news" checked="checked" onclick="update_elements_and_action('insert','update','hidden_action')">Insert
				<input type="radio" name="insert_or_update" id="update" value="update_campus_news" onclick="update_elements_and_action('insert','update','hidden_action')">Update
			</div>
